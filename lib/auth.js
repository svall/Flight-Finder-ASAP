...
function logIn(req, res, next) {
  ...
  // console.log('this is the password ', userPayload.password);
  userModel.getUserByUsername(userPayload.username)
  .then((dbUser) => {
    const matches = bcrypt.compareSync(userPayload.password, dbUser.password);

    // console.log(dbUser.password, matches);
    if (matches) {
      req.session.userId = dbUser._id; //this is what gets saved from the user in the db
      res.user = dbUser;
      // console.log('this is the user id for the session ', res.user);
      next();
    } else {
      res.redirect('/');
    }
